...
# Functions for batteries bellow updated for new Li-Ion batteries
...
bat_acm_B3 = np.array([95, 107, 135]) # B3 values
bat_acm_pct = np.array([0, 30, 100]) # WARNING: Never go below 30%!!!
bat_acm_interp1d = interpolate.interp1d(bat_acm_B3, bat_acm_pct, bounds_error=False, fill_value=0)
...
bat_cmp_volt = np.array([93, 104, 131]) # B2 values
bat_cmp_pct = np.array([0, 30, 100]) # WARNING: Never go below 30%!!!
bat_cmp_interp1d = interpolate.interp1d(bat_cmp_volt, bat_cmp_pct, bounds_error=False, fill_value=0)
...
def process_CODE_AEROCOMP_STATS(self, rf_data, gen_ts, sent_ts, recv_ts, addr):
    ...
    if Id == CODE_AEROCOMP_STATS:
        ...
        batpct = int(bat_cmp_interp1d(B2))
...
def process_CODE_AC_MODEL_STATS(self, rf_data, gen_ts, sent_ts, recv_ts, addr):
    ...
    if Id == CODE_AC_MODEL_STATS:
        ...
        batpct = int(bat_acm_interp1d(B3))
